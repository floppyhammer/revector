...
class MyNode : public Node {
    void custom_ready() override {
        ...
        // Set up tree items.
        {
            ...
            tree_root->set_icon(std::make_shared<VectorImage>(get_asset_dir("icons/Node_Node.svg")));
            auto child_ui = tree->create_item(tree_root, "NodeUi");
            child_ui->set_icon(std::make_shared<VectorImage>(get_asset_dir("icons/Node_Control.svg")));
            auto child_label = tree->create_item(child_ui, "Label");
            child_label->set_icon(std::make_shared<VectorImage>(get_asset_dir("icons/Node_Label.svg")));
            auto child_text_edit = tree->create_item(child_ui, "TextEdit");
            child_text_edit->set_icon(std::make_shared<VectorImage>(get_asset_dir("icons/Node_LineEdit.svg")));
            auto child_node_2d = tree->create_item(tree_root, "Node2d");
            child_node_2d->set_icon(std::make_shared<VectorImage>(get_asset_dir("icons/Node_Node2D.svg")));
            auto child_node_3d = tree->create_item(tree_root, "Node3d");
            child_node_3d->set_icon(std::make_shared<VectorImage>(get_asset_dir("icons/Node_Node3D.svg")));
        }
    }
};
